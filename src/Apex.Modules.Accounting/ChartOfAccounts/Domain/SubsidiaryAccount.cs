...
public sealed class SubsidiaryAccount
{
    ...
    public static SubsidiaryAccount Create(
        long id, long parentId, string code, string name, AccountNature nature,
        DetailAccountType detailType, DateTime now)
    {
        Validate(parentId, code, name);
        return new SubsidiaryAccount
        {
            Id = id,
            GeneralAccountId = parentId,
    ...
    internal static SubsidiaryAccount Rehydrate(
        long id, long parentId, string code, string name, AccountNature nature, DetailAccountType detailType,
        AccountStatus status, DateTime createdAt, DateTime? updatedAt, DateTime? archivedAt) => new()
        {
            Id = id,
            GeneralAccountId = parentId,
            Code = code,
    ...
    public void Archive(DateTime now)
    {
        if (Status == AccountStatus.Archived)
            throw Rule("Subsidiary account is already archived.", ChartOfAccountsErrors.AlreadyArchived);
    ...
    public void Reactivate(DateTime now)
    {
        if (Status == AccountStatus.Active)
            throw Rule("Subsidiary account is already active.", ChartOfAccountsErrors.AlreadyActive);
        Status = AccountStatus.Active;
        UpdatedAt = now;
        ArchivedAt = null;
    }
    ...
    {
        if (parentId <= 0)
            throw Rule("General account is required.", ChartOfAccountsErrors.GeneralAccountNotFound);
