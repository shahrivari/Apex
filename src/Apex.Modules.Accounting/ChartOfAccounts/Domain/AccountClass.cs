...
public sealed class AccountClass
{
    ...
    internal static AccountClass Rehydrate(
        long id, string code, string name, AccountStatus status,
    ...
    public void Archive(DateTime now)
    {
        if (Status == AccountStatus.Archived)
            throw Rule("Account class is already archived.", ChartOfAccountsErrors.AlreadyArchived);
    ...
    public void Reactivate(DateTime now)
    {
        if (Status == AccountStatus.Active)
            throw Rule("Account class is already active.", ChartOfAccountsErrors.AlreadyActive);
        Status = AccountStatus.Active;
        UpdatedAt = now;
        ArchivedAt = null;
    }
    ...
    private static void Validate(string code, string name)
    {
        if (string.IsNullOrWhiteSpace(code) || code.Trim().Length > 64)
            throw Rule("Account code must contain at most 64 characters.", ChartOfAccountsErrors.InvalidCode);
        ValidateName(name);
    }
