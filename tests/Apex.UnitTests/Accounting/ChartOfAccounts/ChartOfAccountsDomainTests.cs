...
public sealed class ChartOfAccountsDomainTests
{
    ...
    {
        var value=AccountClass.Create(1,"A","Name",Now); value.Archive(Now.AddMinutes(1));
        Assert.Equal(AccountStatus.Archived,value.Status); Assert.NotNull(value.ArchivedAt);
        Assert.Equal("account_already_archived",Assert.Throws<BusinessRuleException>(()=>value.Archive(Now)).ErrorCode);
        value.Reactivate(Now.AddMinutes(2)); Assert.Equal(AccountStatus.Active,value.Status); Assert.Null(value.ArchivedAt);
