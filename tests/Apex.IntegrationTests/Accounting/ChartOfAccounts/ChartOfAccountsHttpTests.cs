...
public sealed class ChartOfAccountsHttpTests(ChartOfAccountsHttpFixture fixture):IAsyncLifetime
{
    ...
    public Task InitializeAsync(){_client=fixture.Factory.CreateClient();return Task.CompletedTask;} public Task DisposeAsync(){_client.Dispose();return Task.CompletedTask;}
    ...
    {
        await fixture.ResetAsync();
        var invalid=await _client.PostAsJsonAsync($"{BaseUrl}/classes",new{code="",name=""}); Assert.Equal(HttpStatusCode.BadRequest,invalid.StatusCode);
        Assert.Equal("validation_failed",(await ReadProblemAsync(invalid)).GetProperty("errorCode").GetString());
        var missing=await _client.GetAsync($"{BaseUrl}/AccountClass/999"); Assert.Equal(HttpStatusCode.NotFound,missing.StatusCode);
    ...
    {
        var response=await _client.GetAsync($"{BaseUrl}/search?pageSize=101"); Assert.Equal(HttpStatusCode.BadRequest,response.StatusCode);
    }
