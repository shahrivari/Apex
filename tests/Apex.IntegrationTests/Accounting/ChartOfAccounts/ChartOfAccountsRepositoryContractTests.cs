...
public sealed class ChartOfAccountsRepositoryContractTests(ApexIntegrationTestFixture fixture):ApexIntegrationTestBase(fixture)
{
    ...
    public async Task Repositories_Should_RoundTrip_All_Entities_In_Deterministic_Order()
    {
        ...
        var classWrite=s.GetRequiredService<IAccountClassWriteRepository>(); var generalWrite=s.GetRequiredService<IGeneralAccountWriteRepository>(); var subsidiaryWrite=s.GetRequiredService<ISubsidiaryAccountWriteRepository>();
        var now=new DateTime(2026,7,17,0,0,0,DateTimeKind.Utc);
        await classWrite.InsertAsync(AccountClass.Create(10,"B","B",now)); await classWrite.InsertAsync(AccountClass.Create(11,"A","A",now));
        ...
        var roots=await s.GetRequiredService<IAccountClassReadRepository>().ListAsync(false); Assert.Equal(["A","B"],roots.Select(x=>x.Code));
        var general=await s.GetRequiredService<IGeneralAccountReadRepository>().GetAsync(20); Assert.NotNull(general); Assert.Equal("CREDITOR",general.Nature);
        var leaf=await s.GetRequiredService<ISubsidiaryAccountReadRepository>().GetAsync(30); Assert.NotNull(leaf); Assert.Equal("PERSON",leaf.DetailAccountType);
    }

    [Fact]
    public async Task Database_Should_Enforce_Code_Scopes_Enums_And_Foreign_Keys()
    {
        await ResetAccountingDatabaseAsync(); await using var connection=CreateAccountingConnection(); await connection.OpenAsync(); var now=DateTime.UtcNow;
